...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN: SADZINN MAKER\n'
            + 'ORG:Owner BRUXINHO;\n'
            + 'TEL;type=CELL;type=VOICE;waid=5514998360726:+55 11 93471-3306\n'
            + 'END:VCARD'

prefix = "!"
name = "~ BRUXINHO  | MODS"
rdaftar = "OBRIGADO POR SE REGISTAR PARA SER UM TEMENTE SADZINNMAKER KSKS😁"
rmenu = "SADZINN DOMINA KARAY:)"
limitt = 10
ban = []
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					userB: `salve Mano ${pushname2} voce nn esta cadastrado no banco de dados do SADZINN MAKER digite ${prefix}daftar`,
		...
		case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('vocês já são amigos do SADZINN MAKER:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```Lista Banido``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```Lista de Premium SADZINN MAKER``` :\n'
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ marcando todo mundo fds *${pushname2}* ✪══`+ teks +'╚═══〘 SADZIN MAKER 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sucesso deleta todo o chat SADZINN MAKER\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *SADZINN MAKER BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão de sucesso')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Não registrado no banco de dados *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[SADZINN MAKER]','aqua'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
